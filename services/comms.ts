...
export class CommsManager {
  ...
  private initializePeer() {
    ...
    const peerId = this.isHost ? `cardroom-session-${this.sessionId}` : undefined;
  ...
  private connectToHost() {
    const hostId = `cardroom-session-${this.sessionId}`;
  ...
  public sendAction(action: Action) {
    const actionWithId = { ...action, senderId: this.clientId };
    if (this.isHost) {
        // Host broadcasting its own action to all clients
        this.broadcastAction(actionWithId);
    } else {
      if (this.hostConnection && this.hostConnection.open) {
        this.hostConnection.send(actionWithId);
      } else {
